...
export function Features() {
  return (
    <div className="relative my-30 overflow-hidden">
      ...
      >
        ...
        <div className="justify-center items-center gap-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mt-8 max-w-7xl scale-80">
          {feats.map((feat) => (
            <motion.div
              initial={{ zoom: 0.95, opacity: 0 }}
              whileInView={{ zoom: 1, opacity: 1 }}
              key={feat.title}
              className="group relative bg-card/40 hover:shadow-accent backdrop-blur-2xl p-4 border hover:border-accent rounded-lg max-w-sm aspect-video overflow-hidden hover:scale-[1.02] transition-all"
            >
              ...
              <h3 className="font-semibold text-2xl">{feat.title}</h3>
